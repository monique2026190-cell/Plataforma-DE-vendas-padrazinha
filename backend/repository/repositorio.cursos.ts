import pool from '../db/pool.js';
import {
  buscarCursosQuery,
  buscarCursoPorIdQuery,
  inserirCursoQuery,
  apagarCursoQuery
} from '../db/queries/cursos.queries.js';

/**
 * Busca todos os cursos no banco de dados.
 */
export const buscarTodosCursos = async () => {
  const client = await pool.connect();
  try {
    const result = await client.query(buscarCursosQuery);
    return result.rows;
  } catch (error) {
    console.error('Erro ao buscar todos os cursos no banco de dados', error);
    throw error;
  } finally {
    client.release();
  }
};

/**
 * Busca um curso pelo seu ID no banco de dados.
 */
export const buscarCursoPorIdRepo = async (id: number) => {
  const client = await pool.connect();
  try {
    const result = await client.query(buscarCursoPorIdQuery, [id]);
    return result.rows[0];
  } catch (error) {
    console.error('Erro ao buscar curso por ID no banco de dados', error);
    throw error;
  } finally {
    client.release();
  }
};

/**
 * Insere um novo curso no banco de dados.
 */
export const inserirCurso = async (curso: { nome: string; descricao: string; capa_curso: string; preco: number; usuario_id: number }) => {
  const client = await pool.connect();
  try {
    const result = await client.query(inserirCursoQuery, [
      curso.nome,
      curso.descricao,
      curso.capa_curso,
      curso.preco,
      curso.usuario_id,
    ]);
    return result.rows[0];
  } catch (error) {
    console.error('Erro ao inserir curso no banco de dados', error);
    throw error;
  } finally {
    client.release();
  }
};


/**
 * Apaga um curso do banco de dados.
 */
export const apagarCursoRepo = async (id: number) => {
  const client = await pool.connect();
  try {
    await client.query(apagarCursoQuery, [id]);
  } catch (error) {
    console.error('Erro ao apagar curso do banco de dados', error);
    throw error;
  } finally {
    client.release();
  }
};
